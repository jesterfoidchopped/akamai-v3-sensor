...
func main() {
	...
	fmt.Println("sensor - Streaming Downloads")
	fmt.Println(strings.Repeat("=", 70))
...
- Writing to disk as data arrives`)
	...
	tmpFile, err := os.CreateTemp("", "sensor_stream_*.bin")
	if err != nil {
	...
	sessionH2 := sensor.NewSession("chrome-latest", sensor.WithForceHTTP2())
	...
	sessionH3 := sensor.NewSession("chrome-latest", sensor.WithForceHTTP3())
