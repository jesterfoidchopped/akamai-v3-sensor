...
Use `SensorHandler` for seamless integration with existing `HttpClient` code:

```csharp
using Sensor;

// Create handler with Chrome fingerprint
using var handler = new SensorHandler(preset: "chrome-latest");
using var client = new HttpClient(handler);

// All HttpClient requests now go through sensor with TLS fingerprinting
...
// SensorHandler streams by default (UseStreaming = true)
using var handler = new SensorHandler(preset: "chrome-latest");
using var client = new HttpClient(handler);
...
// With SensorHandler
using var handler = new SensorHandler(preset: "chrome-latest", proxy: "http://user:pass@host:port");
using var client = new HttpClient(handler);
```
...
// With SensorHandler
using var handler = new SensorHandler(
    preset: "chrome-latest",
    echConfigDomain: "cloudflare-ech.com"
);
```
...
// Access cookies via SensorHandler
using var handler = new SensorHandler(preset: "chrome-latest");
...
var presets = SensorInfo.AvailablePresets();
...
{
    // SensorException is wrapped in HttpRequestException
    Console.WriteLine($"Request failed: {ex.Message}");
}
```
