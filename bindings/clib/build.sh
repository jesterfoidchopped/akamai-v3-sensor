#!/bin/bash
# Build script for sensor shared library
...
build_for_platform() {
    ...
    local output="$DIST_DIR/libsensor-${os}-${arch}${ext}"
    ...
    local header_basename="libsensor-${os}-${arch}.h"
    if [ -f "$DIST_DIR/$header_basename" ]; then
        mv "$DIST_DIR/$header_basename" "$DIST_DIR/sensor.h"
    fi
...
copy_to_bindings() {
    ...
    local lib="$DIST_DIR/libsensor-${TARGET_OS}-${TARGET_ARCH}${ext}"

    if [ -f "$lib" ]; then
        echo "Copying to Python bindings..."
        mkdir -p "$SCRIPT_DIR/../python/sensor/lib"
        cp "$lib" "$SCRIPT_DIR/../python/sensor/lib/"
        # Also copy to top-level for direct import (search path priority)
        cp "$lib" "$SCRIPT_DIR/../python/sensor/"
        ...
        if [ -d "$SCRIPT_DIR/../nodejs/node_modules/@sensor/linux-x64" ]; then
            cp "$lib" "$SCRIPT_DIR/../nodejs/node_modules/@sensor/linux-x64/"
        fi

        echo "Copying to .NET bindings..."
        mkdir -p "$SCRIPT_DIR/../dotnet/Sensor/lib"
        cp "$lib" "$SCRIPT_DIR/../dotnet/Sensor/lib/"
        cp "$lib" "$SCRIPT_DIR/../dotnet/Sensor/"
        mkdir -p "$SCRIPT_DIR/../dotnet/Sensor/runtimes/linux-x64/native"
        cp "$lib" "$SCRIPT_DIR/../dotnet/Sensor/runtimes/linux-x64/native/"
...
case "${1:-native}" in
    ...
    clean)
        ...
        rm -rf "$DIST_DIR"
        rm -rf "$SCRIPT_DIR/../python/sensor/lib"
